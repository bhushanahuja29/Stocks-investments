"""Trading-session rules for pinned scrip price polling and alert sounds."""
...
def pin_schedule_kind(symbol: str, market_type: str) -> str:
    """
    nse          — Indian equities: weekdays 9:00–15:30 IST
    crypto_24_7  — Crypto: always on
    forex_weekday — Gold / forex: all day on weekdays
    """
...
def pin_session_status(symbol: str, market_type: str, dt: datetime | None = None) -> str:
    """Short UI hint when polling/alerts are paused."""
    kind = pin_schedule_kind(symbol, market_type)
    ...
    if kind == "nse":
        if not is_weekday_ist(dt):
            return "NSE closed (weekend) — live updates resume Monday 9:00 AM IST."
        if dt.time() < NSE_OPEN:
            return "NSE pre-open — live updates from 9:00 AM IST."
        return "NSE closed — live updates resume next weekday 9:00 AM IST."

    if kind == "forex_weekday":
        return "Gold market pause (weekend) — live updates resume Monday."
